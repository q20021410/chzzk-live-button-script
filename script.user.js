...
function $addLiveButtonToSearchPage () {
  ...
  for (const profileContainer of profileContainers) {
    ...
    const profileThumbnail = profileContainer.querySelector('a[class^="channel_item_wrapper"][href^="/"]');
    const pathnameWithHash = profileThumbnail ? profileThumbnail.getAttribute('href') : null;
    const channelHashMatch = pathnameWithHash ? pathnameWithHash.match(/[a-z0-9]{32}/) : null;
